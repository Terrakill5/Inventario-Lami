...
const Venta = () => {
  ...
  const exportarExcel = () => {
    ...
    const aoa = [
      ...
      [],
      ["NOTA DE ENTREGA"],
      [`CLIENTE: ${nombreCliente}`, , , , , , , `FECHA:${fecha} `],
      ["DIRECCION:", , , , `${direccion}`],
      ["CI: ", `${documentoCliente}`, , "TELEFONO:", , `${telefono}`],
    ...
    if (productos.length >= 0) {
      ...
      while (aoa.length <= 18) {
        aoa.push([`${aoa.length - 7}`]);
      }
      aoa.push([, , "SUB TOTAL", "Bs.", , , , `${subTotal}`]);
    }

    let ws = XLSX.utils.aoa_to_sheet(aoa);
    /* create workbook and export */
    let wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, `${nombreCliente}`);
    XLSX.writeFile(wb, `Venta de ${nombreCliente}.xlsx`);
  };

  const handlePrecio = (index, valor) => {
    let calculo = 0;
    const nextCounters = productos.map((item, i) => {
      if (i === index) {
        ...
        return {
          ...item,
          precio: valor,
          total: (valor * Number(item.cantidad)).toFixed(2),
        };
      } else {
    ...
    nextCounters.map((item, i) => {
      
      calculo = calculo + Number(item.total);
    });
    
    setSubTotal(calculo.toFixed(2));
    setTotal(calculo.toFixed(2));
    setProductos(nextCounters);
  };
                            ...
                            <tr key={item.idProducto}>
                              ...
                              <td>
                                <Input
                                  ...
                                  value={item.precio}
                                  onChange={(e) =>
                                    handlePrecio(index, e.target.value)
                                  }
                                />
                              </td>
                              <td>{item.total}</td>
